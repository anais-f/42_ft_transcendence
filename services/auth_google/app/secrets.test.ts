...
const readSecretMock: jest.MockedFunction<(name: string) => string | undefined> = jest.fn()
...
describe('auth_google getGoogleCredentials', () => {
  ...
  test('uses secrets when available', () => {
    readSecretMock.mockImplementation((name) => (name === 'google_client_id' ? 'sid' : name === 'google_client_secret' ? 'ssecret' : undefined))
