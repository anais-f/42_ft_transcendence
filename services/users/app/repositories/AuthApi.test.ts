...
describe('AuthApi', () => {
	...
	describe('2. Basic failure cases', () => {
		...
		test('throws Error when fetch fails with network error', async () => {
			...
			await expect(AuthApi.getAllUsers()).rejects.toThrow(
				'Failed to fetch users from auth: Network error'
			)
		})

		test('throws GatewayTimeout when request times out', async () => {
		...
		test('throws GatewayTimeout when request is aborted', async () => {
