...
describe('usersControllers', () => {
  ...
  describe('2. Basic failure cases', () => {
    ...
    test('getPublicUser: returns 400 for invalid user_id (0, negative, non-numeric)', async () => {
      ...
      await getPublicUser({ params: { id: '0' } } as unknown as FastifyRequest, mockReply)
      expect(mockReply.code).toHaveBeenCalledWith(400)

      jest.clearAllMocks()
      await getPublicUser({ params: { id: '-1' } } as unknown as FastifyRequest, mockReply)
      expect(mockReply.code).toHaveBeenCalledWith(400)

      jest.clearAllMocks()
      await getPublicUser({ params: { id: 'invalid' } } as unknown as FastifyRequest, mockReply)
      expect(mockReply.code).toHaveBeenCalledWith(400)
    })
  ...
  describe('4. Exceptions', () => {
    ...
    test('getPublicUser: throws generic errors (not caught)', async () => {
      ...
      await expect(getPublicUser(mockRequest, mockReply)).rejects.toThrow('Unexpected error')
    })
  })
})
