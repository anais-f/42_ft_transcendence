...
`playerToGame`. The `games` map stores all games, identified by a unique code
generated by `createInviteCode('G')`. Each game contains information about both
players (`p1` and `p2`), the game instance, a status, and a creation timestamp.
Each player is represented by the `Iplayer` interface which combines their
identifier and their WebSocket connection (`ws`). The
`playerToGame` map provides a reverse lookup to find a player's current game.
...
| Error                             | Cause                                |
| --------------------------------- | ------------------------------------ |
| `'a player is already in a game'` | `pID1` or `pID2` already has an game |

### `joinGame(gameCode, playerId)`

Attempts to add/connect a player to an existing game. On success, the
associated route will handle JWT token generation for WebSocket connection.

- **If `playerId === p1.id`**:
  - If `p1.ws` is `null`: allowed (player can join)
  - If `p1.ws` is set: throws `'player is already in a game'`
- **If `playerId === p2.id` (locked game)**:
  - If `p2.ws` is `null`: allowed (player can join)
  - If `p2.ws` is set: throws `'player is already in a game'`
- **Open game (`p2` is undefined)**:
  - Assigns player as `p2` with `ws: null`
...
| `'player is already in a game'`     | Player's `ws` is already set           |
| `'player not allowed in this game'` | game is locked                         |
...
| `joinGame`    | `'player is already in a game'`     | Player's `ws` is already set                              |
