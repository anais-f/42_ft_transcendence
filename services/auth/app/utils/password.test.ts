...
const hashMock: jest.MockedFunction<(password: string, opts: any) => Promise<string>> = jest.fn()
const verifyMock: jest.MockedFunction<(hash: string, password: string) => Promise<boolean>> = jest.fn()
...
describe('password utils', () => {
  ...
  test('hashPassword passes argon2id and returns hash', async () => {
    ...
    expect(hashMock).toHaveBeenCalledWith('supersecret', expect.objectContaining({ type: 2 }))
  })
