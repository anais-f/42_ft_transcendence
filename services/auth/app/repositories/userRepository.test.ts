...
describe('userRepository (integration with in-memory DB)', () => {
	...
	test('listPublicUsers returns array', () => {
		...
		expect(list?.users.map((u) => u.login)).toEqual(
	...
	test('deleteUserById removes a row', () => {
		...
		const { user_id } = db
			.prepare('SELECT user_id FROM users WHERE login=?')
			.get('to-del') as { user_id: number }
