...
const createUserMock: jest.MockedFunction<(login: string, hash: string) => void> = jest.fn()
const findUserByLoginMock: jest.MockedFunction<(login: string) => { user_id: number; login: string; password: string; is_admin: boolean } | undefined> = jest.fn()
const createAdminUserMock: jest.MockedFunction<(login: string, hash: string) => void> = jest.fn()
const createGoogleUserMock: jest.MockedFunction<(googleId: string, login: string, avatar?: string) => { user_id: number; login: string; password: string; is_admin: boolean }> = jest.fn()
const hashPasswordMock: jest.MockedFunction<(pwd: string) => Promise<string>> = jest.fn()
const verifyPasswordMock: jest.MockedFunction<(hash: string, pwd: string) => Promise<boolean>> = jest.fn()
...
const { loginUser, registerUser, registerAdminUser, registerGoogleUser } = await import('./register.js')
...
describe('auth register/login usecases', () => {
	...
	test('loginUser returns null if password missing', async () => {
		...
		test('loginUser returns token when credentials valid', async () => {
			findUserByLoginMock.mockReturnValue({ user_id: 5, login: 'alice', password: 'hash', is_admin: false } as any)
		...
		const res = await loginUser('alice', 'pw')
			...
			expect(signTokenMock).toHaveBeenCalledWith({ user_id: 5, login: 'alice', is_admin: false })
