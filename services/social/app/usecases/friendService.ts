...
export class FriendService {
  static async sendFriendRequest(userId: IUserId, friendId: IUserId): Promise<void> {
    if (userId === friendId)
    ...
    const existingRelation = SocialRepository.getRelationStatus(userId, friendId)
    if (existingRelation === 1)
      throw new AppError(ERROR_MESSAGES.RELATION_ALREADY_EXIST, 400)
  ...
  static async acceptFriendRequest(userId: IUserId, friendId: IUserId): Promise<void> {
  ...
  static async rejectFriendRequest(userId: IUserId, friendId: IUserId): Promise<void> {
    const status = SocialRepository.getRelationStatus(userId, friendId)
    if (status !== 0)
      throw new AppError(ERROR_MESSAGES.NO_PENDING_REQUEST, 404)
  ...
  static async cancelFriendRequest(userId: IUserId, friendId: IUserId): Promise<void> {
    const status = SocialRepository.getRelationStatus(userId, friendId)
    if (status !== 0)
      throw new AppError(ERROR_MESSAGES.NO_PENDING_REQUEST, 404)
