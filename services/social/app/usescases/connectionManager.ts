 ...
 */
function notifyStatusChange(userId: string, status: 'online' | 'offline'): void {
  // TODO: Call PATCH /api/internal/users/:id/status endpoint here
  console.log(`[STATUS] User ${userId} is now ${status.toUpperCase()} (after ${DISCONNECT_DELAY_MS}ms delay)`)
}
...
export function addConnection(userId: string, ws: WebSocket): boolean {
  ...
  if (!isFirstConnection) {
    ...
    try {
      ...
      console.error(`Error closing previous WebSocket for user ${userId}:`, message)
    }
  }
...
export function removeConnection(userId: string, ws: WebSocket): boolean {
  ...
  console.log(`User ${userId} disconnected, offline in ${DISCONNECT_DELAY_MS}ms`)
...
export function broadcast(message: unknown): void {
  ...
  console.log(`Broadcast sent to ${totalSent} users, with ${wsConnections.size - totalSent} failures`)
}
