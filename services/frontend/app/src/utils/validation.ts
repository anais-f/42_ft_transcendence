...
export function validateUsername(username: unknown) {
	...
	if (!validation.success) {
		const errorMessage = validation.error.errors?.[0]?.message || 'Login must be 4-32 characters long and can include letters, numbers, underscores, and hyphens'
...
export function validatePassword(password: unknown) {
	...
	if (!validation.success) {
		const errorMessage = validation.error.errors?.[0]?.message || 'Password must be 8-128 characters long and include at least one letter and one number'
		return { success: false, error: errorMessage }
	}
	return { success: true, data: validation.data }
}
