		...
		describe('Segment', () => {
			describe('basic test', () => {
				...
				test('diagonal', () => {
					...
					const s: Segment[] = []
					v.forEach((a, i) =>
						v.slice(i + 1).forEach((b) => s.push(new Segment(a, b)))
					)
				...
				test('outside', () => {
					...
					outsidePoints.forEach((a, i) =>
						outsidePoints.slice(i + 1).forEach((b) => s.push(new Segment(a, b)))
					)
					s.forEach((seg) => expect(w.isInside(seg)).toBe(false))
				})

				test('both', () => {
					const insidePoints = [MM, TL, TR, TM, BL, BR, BM, ML, MR]
					const outsidePoints = [
						new Vector2(-25, 0),
						new Vector2(25, 0),
						new Vector2(0, 15),
				...
				test('crossing rectangle', () => {
					expect(
						w.isInside(new Segment(new Vector2(-100, 0), new Vector2(100, 0)))
					).toBe(false)
				})
			})
		})
