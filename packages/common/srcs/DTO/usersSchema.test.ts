...
import { RegisterLoginSchema, UserIdSchema, PublicUserAuthSchema, UserPublicProfileSchema } from './usersSchema.js'
...
describe('usersSchema', () => {
  ...
  test('UserIdSchema validates correct length and rejects wrong size', () => {
    const ok = UserIdSchema.safeParse({ id: 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa' })
  ...
  test('PublicUserAuthSchema allows undefined optional fields and rejects too short login', () => {
    expect(PublicUserAuthSchema.safeParse({ login: 'publicUser' }).success).toBe(true)
  ...
  test('UserPublicProfileSchema accepts minimal profile and rejects short login', () => {
    expect(UserPublicProfileSchema.safeParse({ login: 'publicUser' }).success).toBe(true)
    expect(UserPublicProfileSchema.safeParse({ login: 'pw' }).success).toBe(false)
  })
})
